...
Listen for updates from ktranslate

Start with up/down for a service/ip

1 service == 1 test.
Each IP in test == option.
** Return IP for each test which is lowest latency
** Return all IPs for each test where latency is less than X, or lowest Y
*/
...
func (h Ksynth) ServeDNS(ctx context.Context, w dns.ResponseWriter, r *dns.Msg) (int, error) {
...
func (h Ksynth) otherRecordsExist(qname string) bool {
...
func (h Ksynth) Name() string { return "hosts" }
...
func (h *Ksynth) ptr(zone string, ttl uint32, names []string) []dns.RR {
